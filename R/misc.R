all.fixtures <- function("team") {
  ...
  Points.Per.Game <- data.frame(MatchWeeks = seq(1:number.of.matchweeks),
...
number.of.matchweeks <- 2*length(grep("Chelsea", football$AwayTeam))
indices.of.home.team <- grep("Chelsea", football$HomeTeam)
indices.of.away.team <- grep("Chelsea", football$AwayTeam)
indices.of.both <- sort(c(indices.of.home.team,indices.of.away.team))
